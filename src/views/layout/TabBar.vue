<template>
  <div
    class="tab-list"
    :style="{
      left: store.asideWidth + 'px'
    }"
  >
    <!-- tab栏 -->
    <el-tabs v-model="activeTab" type="card" class="demo-tabs" @tab-remove="removeTab" @tab-change="changeTab" style="min-width: 100px">
      <el-tab-pane :closable="item.path != '/home'" v-for="item in tabLists" :key="item.path" :label="item.title" :name="item.path"> </el-tab-pane>
    </el-tabs>
    <!-- 下拉菜单 -->
    <span class="dropdown">
...
import { useTabList } from '@/hooks/useTabBar.js'
